...
#  main.py --with_box_refine --binary --freeze_text_encoder --visualize --cat_id=${i} --dataset_file gygo \
...
PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
  python3 -m torch.distributed.launch --nproc_per_node=1 --master_port=${PORT} --use_env \
  main_gao.py --with_box_refine --binary --freeze_text_encoder --visualize   --dataset_file gygo \
  --epochs 6 --lr_drop 3 5 \
  --output_dir=${OUTPUT_DIR} --pretrained_weights=${PRETRAINED_WEIGHTS} ${PY_ARGS}
