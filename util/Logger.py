...
class Loss_record():
    ...
    def getloss(self, epoch, step):
        ...
        self.reset()
        return out_str, total_avg, part1_avg, part2_avg
...
from util.davis_JF import db_eval_boundary, db_eval_iou
    ...
    def update_evl(self, idx, query_mask, pred):
        ...
        for i in range(batch):
            ...
            for j in range(query_mask[i].shape[0]):
                ...
                self.tmp_f += db_eval_boundary(predict, y)
                self.f_list[id] += db_eval_boundary(predict, y)
                self.tmp_j += db_eval_iou(y, predict)
