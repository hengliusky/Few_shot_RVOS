...
class SAILVOSDataset(Dataset):
    def __init__(self, data_path=None, train=True, valid=False,
                 set_index=3, finetune_idx=None,
                 support_frame=5, query_frame=1, sample_per_class=100,
                 transforms=None, another_transform=None, cat_id=1):
        self.train = train
        self.valid = valid
        ...
        for i in range(len(self.class_list)):
            ...
            for j in range(len(self.video_ids[i]) - 1):
    ...
    def get_GT_byclass(self, vid, class_id, frame_num=1, test=False):
        ...
        # return imgs, masks, exp, choice_frame
    ...
    def __gettestitem__(self, idx):
        ...
        if begin_new:
            ...
            if len(self.support_vids) < 3:
                self.support_vids.append(support_vid[0])
            query_vids = []
        ...
        # return query_frames, query_masks, query_exps, support_frames, support_masks, support_exps, self.class_list[
        #     list_id], vid_name, begin_new, frames_name
        # if self.train:
        #     return imgs, targets
        # else:
        #     return qimgs, qtargets
        if not begin_new:
